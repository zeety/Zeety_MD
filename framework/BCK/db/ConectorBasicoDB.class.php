...
class ConectorBasicoDB
{
	...
	function __construct($configuracion){
		...
		if(isset($this->configuracion["db_nombre"])
				&& isset($this->configuracion["db_usuario"])
				&& isset($this->configuracion["db_clave"])
				&& isset($this->configuracion["db_sys"])
				&& isset($this->configuracion["db_dns"])
				&& isset($this->configuracion["db_prefijo"])
		){
			$this->especificar_db($this->configuracion["db_nombre"]);
			$this->especificar_usuario($this->configuracion["db_usuario"]);
			$this->especificar_clave($this->configuracion["db_clave"]);
			$this->especificar_servidor($this->configuracion["db_dns"]);
			$this->especificar_dbsys($this->configuracion["db_sys"]);
			$this->conectar_db();
		}
	}
	...
	function conectar_db()
	{
		switch($this->dbsys)
		{
			...
			// TODO: Soportar los otros tipos de base de datos
			case 'mysql':
				if($this->conectarMysql()){
					return $this->enlace;					
				}
				break;

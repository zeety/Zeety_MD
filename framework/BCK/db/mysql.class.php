...
class mysql
{
	...
	function __construct($registro)
	{
		if(isset($registro[0][1])){
			...
			$this->servidor = $registro["db_dns"];
			$this->db = $registro["db_nombre"];
			$this->usuario = $registro["db_usuario"];
			$this->clave = $registro["db_clave"];
			$this->dbsys = $registro["db_sys"];
		}
	...
	function ejecutarAcceso($configuracion,$cadena_sql,$tipo="")
	{
		if(!is_resource($this->enlace)&&$enlace==""){
			echo "NO HAY ACCESO A LA BASE DE DATOS!!!";
			return FALSE;
		}
		...
		if($tipo=="busqueda"){
			...
			if($configuracion["debugMode"]==1&&$esteRegistro==false){
				echo "El registro esta vacio!!!<br>";
			}

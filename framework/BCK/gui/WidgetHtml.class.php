...
class  WidgetHtml extends FuncionGeneral {
	/** Aggregations: */
	...
	function cuadro_lista($configuracion, $atributos) {
		...
		include_once($this->configuracion["raiz_documento"].$this->configuracion["clases"]."/cadenas.class.php");
		$this->formato=new cadenas();

		//Invocar la funcion que rescata el registro de los valores que se mostraran en la lista
		$this->rescatarRegistroCuadroLista();
	...
	private function rescatarRegistroCuadroLista(){
		...
		if(!is_array($cuadro_sql)) {
			...
			if(!isset($this->atributos["baseDatos"]) || (isset($this->atributos["baseDatos"])&&$this->atributos["baseDatos"]=="")) {
				$this->atributos["baseDatos"]= $this->configuracion["db_estructura"];
			}


			$this->conectarDB($this->configuracion, $this->atributos["baseDatos"]);

			//En el caso que se requiera una minilista de opciones al principio
			if(isset($this->atributos["subcadena_sql"])) {

				$miniRegistro=$this->ejecutarSQL($this->configuracion, $this->atributos["subcadena_sql"],"busqueda", $this->atributos["baseDatos"]);
				$totalMiniRegistro=$this->totalRegistros($this->configuracion,$this->atributos["baseDatos"]);
			}

			$this->cuadro_registro=$this->ejecutarSQL($this->configuracion, $this->atributos["cadena_sql"],"busqueda", $this->atributos["baseDatos"]);
			$this->cuadro_campos=$this->totalRegistros($this->configuracion,$this->atributos["baseDatos"]);
		}else {
